...
async function LoadData(){
    let response = await fetch(`${API_URL}/${inputVersion.value}/${inputBook.value}/${inputChapter.value}/`) //RV1960
...
const createUserItems = data => data.map(bible => `<ol>${bible.verse}. ${bible.text}</ol>`).join(' ')
...
function RenderData(data){
    ...
    const itemsString = createUserItems(data)
...
input.addEventListener('keyup', event => {
    const newData = data.filter(data => `${data.text.toLowerCase()}`.includes(input.value.toLowerCase()))
    RenderData(newData)
})
